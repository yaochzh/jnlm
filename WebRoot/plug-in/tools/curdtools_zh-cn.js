...
function srcAdd(title,url,id,gname,width,height,isRestful) {
	...
	gridname=gname;
	createwindowU8(title, url,width,height);
}
...
function update(title,url, id,width,height,isRestful) {
	...
	if (rowsData.length>1) {
	 ...
	 if(rowsData[0].billstatus == 1 || rowsData[0].billstatus == -1 ) {
		tip("已审核的单据不能编辑！");
		return;
	}
	if(isRestful!='undefined'&&isRestful){
 ...
 * @return
 */
function approveSelect(title,url,gname,width,height) {
	gridname=gname;
    var ids = [];
    var rows = $("#"+gname).datagrid('getSelections');
    if (rows.length > 0) {
    	...
    	url += '&load=detil&ids='+ids;
    	//openwindow(title,url,width,height);
    	createapprovewindow(title,url,width,height)
    	//createwindow(title,url,width,height);
	} else {
		tip("请选择需要审核下单的数据");
	}
 ...
 * @param saveurl
 */
function createwindowU8(title, addurl,width,height) {
	width = width?width:700;
	height = height?height:400;
	if(width=="100%" || height=="100%"){
		width = window.top.document.body.offsetWidth;
	...
	if(typeof(windowapi) == 'undefined'){
		$.dialog({
		    ...
		    ok: function(){
		    	iframe = this.iframe.contentWindow;
				saveObj();
				$('#soPickupplanList').datagrid('reload',{});
				$('#poDeliveryplanList').datagrid('reload',{});
				return false;
		    },
		    cancelVal: '关闭',
		    cancel: true /*为true等价于function(){}*/
 ...
 * @param saveurl
 */
function createapprovewindow(title, addurl,width,height) {
	width = width?width:700;
	height = height?height:400;
	if(width=="100%" || height=="100%"){
		width = window.top.document.body.offsetWidth;
	...
	if(typeof(windowapi) == 'undefined'){
		$.dialog({
			...
			button : [ {
				...
				callback : function() {
					iframe = this.iframe.contentWindow;
			    	$("#approveStatus", iframe.document).val("1");
			    	saveObj();
			    	return false;
				...
				callback : function() {
					iframe = this.iframe.contentWindow;
			    	$("#approveStatus", iframe.document).val("2");
			    	saveObj();
			    	return false;
				}
			} ]
