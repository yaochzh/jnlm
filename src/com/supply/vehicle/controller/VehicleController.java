...
//			}
		...
		if(request.getParameter("bsealflag") == null) {
		    cq.eq("bsealflag", "N");
		}
		cq.add();
	...
	public AjaxJson doAdd(VehicleEntity vehicle, HttpServletRequest request) {
		...
		try{
		    ...
		    String hql = "select id\n" + "  from bd_vehicle\n"
			    + " where vehiclelicense = '"+vehicle.getVehiclelicense()+"' and  create_by = '"+createby+"'" ;
	...
	public AjaxJson doUpdate(VehicleEntity vehicle, HttpServletRequest request) {
		...
		try {
		    ...
		    // 添加编码和名称重复校验
		    String hql = "select id\n" + "  from bd_vehicle\n"
			    + " where vehiclelicense = '"+vehicle.getVehiclelicense()+"' and create_by = '"+createby+"' and id <> '"+vehicle.getId()+"'" ;
		    List<BdCarrierEntity> list = systemService.findListbySql(hql);
		    if (list != null && list.size() > 0) {
			message = "车牌号已存在！";
			throw new BusinessException(message);
	...
	public String exportXls(VehicleEntity vehicle,HttpServletRequest request,HttpServletResponse response
		...
		modelMap.put(NormalExcelConstants.PARAMS,new ExportParams("车辆档案列表", "导出人:"+ResourceUtil.getSessionUserName().getRealName(),
			"导出信息"));
