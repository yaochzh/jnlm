...
//---author:jg_xugj----end-----date:20151210--------for：#781  【oracle兼容】兼容问题fun.operation!='' 在oracle 数据下不正确
						List<Object> hasOperList = this.systemService.findListbySql(hasOperSql); 
					    for(Object operationIds:hasOperList){
							    for(String operationId:operationIds.toString().split(",")){
